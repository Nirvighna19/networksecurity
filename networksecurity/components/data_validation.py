...
class DataValidation:
    ...
    def validate_numerical_columns(self , dataframe: pd.DataFrame)-> bool:
        try:
            ...
            numerical_col=len(self._schema_config['numerical_columns'])
            logging.info(f"Required number of numerical columns: {numerical_col}")

            numerical_col_of_dataframe=[feature for feature in dataframe.columns if dataframe[feature].dtype!='O']
            logging.info(f"Data frame has numerical columns : {len(numerical_col_of_dataframe)}")

            if numerical_col==len(numerical_col_of_dataframe):
                return True
            return False
